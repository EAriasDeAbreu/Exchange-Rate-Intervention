...
gen mon_d_vol_pur =.
gen mon_d_vol_sal =.
gen mon_d_vol_garch_pur =.
gen mon_d_vol_garch_sal =.
...
forvalues i = 2/18{
replace vol_garch_pur = vol_garch_1_`i' if ene == `i'
replace mon_d_vol_garch_pur = mont_vol_pur_`i' if ene == `i'
}
forvalues i = 2/14{
replace vol_garch_sal = vol_garch_2_`i' if ene == (`i')
replace mon_d_vol_garch_sal = mont_vol_sal_`i' if ene == `i'
}
...
***GRAFICA DESACUMULACION DE RESERVAS***
sum res_sal,d
...
***GRAFICA COMPRA OPCIONES VOLATILIDAD***
sum vol_pur,d
...
***GRAFICA VENTA OPCIONES VOLATILIDAD***
sum vol_sal,d
...
***GRAFICA DISCRECIONALES FILTRADAS***
preserve
...
***GRAFICA GARCH SOLO VENTAS***
sum vol_garch_sal ,d
twoway (kdensity vol_garch_sal, range(-4 2) lcolor(black) xline(-0.4420475 , lc(stblue)) lc(gs2)) || ///
       (function y=0, range(0 0) lcolor(stblue)), ///
       legend(order(1 "Density" 2 "Mean")) ///
	   title("Volatility sales options effects on volatility measured with GARCH") ///
	   xtitle("SC effects distribution") xlabel(,nogrid) ///
	   ylabel(,nogrid) ytitle("Density")
graph export "graficas/vol_garch_sal.png", replace
...
display "VOL GARCH SAL"
display med " " sd " " t " " obs

***GRAFICA GARCH SOLO COMPRAS***
sum vol_garch_pur,d
twoway (kdensity vol_garch_pur, range(-2 3) lcolor(black) xline(0.2627673, lc(stblue)) lc(gs2)) || ///
       (function y=0, range(-0 0) lcolor(stblue)), ///
       legend(order(1 "Density" 2 "Mean")) ///
	   title("Volatility purchase options effects on volatility measured with GARCH") ///
	   xtitle("SC effects distribution") xlabel(,nogrid) ///
	   ylabel(,nogrid) ytitle("Density")
graph export "graficas/vol_garch_pur.png", replace
